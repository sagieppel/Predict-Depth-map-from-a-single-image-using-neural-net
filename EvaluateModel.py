...
import cv2
import XYZScaleFunctions
import open3d as o3d
import RGBD_To_XYZMAP
############################CREATE XYZ loss class ########################################################################################################

LossXYZ=XYZScaleFunctions.Loss(TrainMode=False)

##################################Input paramaters#########################################################################################

#.................................Main Input parametrs...........................................................................................
Sun3EvalDDir=r"/media/breakeroftime/2T/Data_zoo/SUN_RGBD/SUNRGBDLSUNTest///"
...
#***************************************************************************************************
 ...
 # #  *****************************************************************************
    ...
    with torch.no_grad():
        LogDepthMap = Net.forward(Images=Imgs, TrainMode=False)  # Run net inference and get prediction
###############################################################################################################
    DepthMap = torch.pow(2.718281, LogDepthMap) # Convert from log depth to depth
    # DepthMap=LogDepthMap
    # ----------------------------Convert Prediction to numpy-------------------------------------------
    DepthMap = DepthMap[0][0].data.cpu().numpy()
   # vis.show(DepthMap*100)
    # -----------------------------Convert Depth to XYZ map-------------------------------------------------------------------
    #Factor = 1000 / DepthMap.max()
    # ***************************
    #
    ...
    # color_raw = o3d.cpu.pybind.geometry.Image(Imgs[0].astype(np.float32))
    # rgbd_image = o3d.geometry.RGBDImage.create_from_sun_format(color_raw, depth_raw)
    XYZ, ROIX, ImGrey, DepthMap = RGBD_To_XYZMAP.Depth2XYZ(Imgs[0],DepthMap, o3d.camera.PinholeCameraIntrinsic(o3d.camera.PinholeCameraIntrinsicParameters.PrimeSenseDefault))
    #vis.show(DepthMap *100 )
 #   vis.showcv2(ROIX*200,"ROI")
    #vis.ShowXYZMap(XYZ)
    #vis.DisplayPointCloud(XYZ, Imgs[0], ROI[0], step=2)
   # vis.ShowXYZMap(GT_XYZ[0])
    PrdXYZ=torch.autograd.Variable(torch.from_numpy(XYZ).unsqueeze(0).to(device),requires_grad=False).transpose(1, 3).transpose(2, 3)
    ####################################################################################################

       #------------------------ROI Punish depth prediction only within mask----------------------------------------------------

    ROI = torch.autograd.Variable(torch.from_numpy(ROI).unsqueeze(1).to(device),requires_grad=False)
  # ROI = nn.functional.interpolate(ROI, tuple((PrdXYZ[nm].shape[2], PrdXYZ[nm].shape[3])),mode='bilinear', align_corners=False)
    ...
    GTXYZ_PT = torch.from_numpy(GT_XYZ).to(device).transpose(1,3).transpose(2,3) ###.unsqueeze(1)
