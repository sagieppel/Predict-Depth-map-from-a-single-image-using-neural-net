#  FCN that predict Depth map from image
import torch
import copy
...
#-------------------------------------Squeeze ASPP Layer------------------------------------------------------------------------------
            ...
            self.LogDepthLayer=nn.Conv2d(256, 1, stride=1, kernel_size=3, padding=1, bias=False)
...
#----------------------------Upsample features map  and combine with layers from encoder using skip  connection-----------------------------------------------------------------------------------------------------------
    ...
    # ---------------------------------Final log depth map prediction-------------------------------------------------------------------------------
                x = self.LogDepthLayer(x)
                ...
                if not TrainMode:# If training predict small version of the the depth map to save memory else predict depth map in image size
                      LogDepthMap = nn.functional.interpolate(x, size=InpImages.shape[2:4], mode='bilinear',align_corners=False)  # Resize to original image size
                else:
                     LogDepthMap=x
                return LogDepthMap
