# Loss function for XYZ used in evaluation
import torch
import copy
...
########################################################################################################################
    def __init__(self,TrainMode=True): # Create class for Loss function for XYZ maps
 ...
 #          type=np.float32
           ...
           # if not TrainMode:
           #     for nm in self.DifLayers:
           #         self.DifLayers[nm]=self.DifLayers[nm].half()
           #         self.ROILayers[nm] = self.ROILayers[nm].half()
